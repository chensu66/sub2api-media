...
func RegisterMediaRoutes(r *gin.Engine, handler *media.Handler, apiKeyAuth middleware.APIKeyAuthMiddleware) {
	...
	group.POST("/uploads", handler.ProxyUpload)
	group.PUT("/uploads/:upload_id/parts/:part_number", handler.ProxyUpload)
	group.POST("/uploads/:upload_id/complete", handler.ProxyUpload)
	group.DELETE("/uploads/:upload_id", handler.ProxyUpload)
