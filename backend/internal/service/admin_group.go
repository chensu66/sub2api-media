...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if group.Platform == PlatformMedia {
		if input.ModelPricing != nil && len(*input.ModelPricing) > 0 {
			return nil, errors.New("media groups use Gate quotes and do not support model pricing")
		}
...
func normalizeGroupModelPricing(platform string, pricing []ChannelModelPricing) ([]ChannelModelPricing, error) {
	if platform == PlatformMedia && len(pricing) > 0 {
		return nil, errors.New("media groups use Gate quotes and do not support model pricing")
	}
